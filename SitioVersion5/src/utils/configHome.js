/**
 * Retrieves the device type based on the mobile property of the appComponent.
...
/**
 * Populates the components array with home components from the appComponent configuration.
 * @param {Object} appComponent - The application component containing configuration.
 * @param {Array} components - The array to populate with components.
 */
export function getComponents(appComponent, components) {
    if (
        ...
        appComponent.config["components"][appComponent.country]["home"].forEach(
            (component) => {
                components.push(component);
            }
        );
...
/**
 * Retrieves a custom message for the bet slip based on the current timestamp.
 * @param {Object} appComponent - The application component containing configuration.
 * @returns {string} - Returns the message if found, otherwise an empty string.
 */
export function getBetSlipCustomMsg(appComponent) {
    let sportbookMessageSchedule = appComponent.config.sportbook_message_schedule !== undefined ? appComponent.config.sportbook_message_schedule : [];
    let currentTimestamp = Date.now()
    let messageData = sportbookMessageSchedule.find(schedule =>
        currentTimestamp >= schedule.startDate && currentTimestamp <= schedule.endDate && !schedule.isExpired
    );
    return messageData ? messageData.message : '';
}

/**
 * Retrieves information icons based on the user's session state and configuration.
 * @param {Object} appComponent - The application component containing session and configuration.
 * @returns {Array|null} - Returns the icons if found, otherwise null.
 */
export function getInfoIcons(appComponent) {
    let INFO_ICONS = null;
    if (appComponent.session && appComponent.session.logueado){
        if (
            appComponent.config &&
            appComponent.config["login"] &&
            appComponent.config["login"][appComponent.lngProd] &&
            appComponent.config["login"][appComponent.lngProd]["NEW_HOME_ICONS"]
        ) {
            INFO_ICONS = appComponent.config["login"][appComponent.lngProd]["NEW_HOME_ICONS"];
        } else if (
            appComponent.mobile &&
...
/**
 * Updates the browser history with the specified path.
 * @param {string} path - The path to push to the history stack.
 */
...
/**
 * Normalizes a slug by replacing spaces with hyphens and removing invalid characters.
 * @param {string} slug - The slug to normalize.
 * @returns {string} - Returns the normalized slug.
 */
...
/**
 * Retrieves images based on the user's session state and device type.
 * @param {Object} appComponent - The application component containing session and configuration.
 * @param {string} device - The device type (e.g., mobile or desktop).
 * @returns {Array|null} - Returns the images if found, otherwise null.
 */
export function getImages(appComponent, device) {
    ...
    if (appComponent.session && appComponent.session.logueado) {
        if (
            ...
            appComponent.config["login"] &&
            appComponent.config["login"][appComponent.lngProd] &&
            appComponent.config["login"][appComponent.lngProd]["banners"] &&
            appComponent.config["login"][appComponent.lngProd]["banners"]["banner_home"]
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config["login"][appComponent.lngProd]["banners"]["banner_home"]
            ...
            appComponent.config[device][appComponent.country][appComponent.lngProd]["login"] &&
            appComponent.config[device][appComponent.country][appComponent.lngProd]["login"]["home"]
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config[device][appComponent.country][appComponent.lngProd]["login"]["home"]
...
/**
 * Retrieves casino images based on the user's session state and device type.
 * @param {Object} appComponent - The application component containing session and configuration.
 * @param {string} device - The device type (e.g., mobile or desktop).
 * @returns {Array|null} - Returns the casino images if found, otherwise null.
 */
export function getImagesCasino(appComponent, device) {
    ...
    if (appComponent.session && appComponent.session.logueado) {
        if (
            ...
            appComponent.config["login"][appComponent.lngProd] &&
            appComponent.config["login"][appComponent.lngProd]["banners"] &&
            appComponent.config["login"][appComponent.lngProd]["banners"]["banner_casino"]
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config["login"][appComponent.lngProd]["banners"]["banner_casino"]
            ...
            appComponent.config[device][appComponent.country][appComponent.lngProd] &&
            appComponent.config[device][appComponent.country][appComponent.lngProd]["login"] &&
            appComponent.config[device][appComponent.country][appComponent.lngProd]["login"]["casino"]
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config[device][appComponent.country][appComponent.lngProd]["login"]["casino"]
...
export function getImagesCasinoLive(appComponent, device) {
    let images = null;
    if (appComponent.session && appComponent.session.logueado) {
        if (
            appComponent.config &&
            ...
            appComponent.config['login'][appComponent.lngProd] &&
            appComponent.config['login'][appComponent.lngProd]['banners'] &&
            appComponent.config['login'][appComponent.lngProd]['banners']['banner_live_casino']
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config['login'][appComponent.lngProd]['banners']['banner_live_casino'],
            ...
            appComponent.config &&
            appComponent.config['components_'] &&
            appComponent.config['components_']['casinoLive']
        ) {
            const carouselComponent = appComponent.config['components_']['casinoLive'].find(
                (item) => item.id === 'Carousel3Component',
            );
            if (carouselComponent && carouselComponent['banners']) {
                images = JSON.parse(JSON.stringify(carouselComponent['banners']));
            ...
            appComponent.config[device][appComponent.country][appComponent.lngProd] &&
            appComponent.config[device][appComponent.country][appComponent.lngProd]['login'] &&
            appComponent.config[device][appComponent.country][appComponent.lngProd]['login']['casinoLive']
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config[device][appComponent.country][appComponent.lngProd]['login']['casinoLive'],
        ...
        if (
            ...
            appComponent.config['not_login'][appComponent.lngProd] &&
            appComponent.config['not_login'][appComponent.lngProd]['banners'] &&
            appComponent.config['not_login'][appComponent.lngProd]['banners']['banner_live_casino']
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config['not_login'][appComponent.lngProd]['banners']['banner_live_casino'],
            ...
            appComponent.config[device][appComponent.country][appComponent.lngProd] &&
            appComponent.config[device][appComponent.country][appComponent.lngProd]['notLogin'] &&
            appComponent.config[device][appComponent.country][appComponent.lngProd]['notLogin']['casinoLive']
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config[device][appComponent.country][appComponent.lngProd]['notLogin']['casinoLive'],
...
export function getImagesVirtuals(appComponent, device) {
    let images = null;
    if (appComponent.session && appComponent.session.logueado) {
        if (
            appComponent.config &&
            ...
            appComponent.config['login'][appComponent.lngProd] &&
            appComponent.config['login'][appComponent.lngProd]['banners'] &&
            appComponent.config['login'][appComponent.lngProd]['banners']['banner_virtual']
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config['login'][appComponent.lngProd]['banners']['banner_virtual'],
            ...
            appComponent.config &&
            appComponent.config['components_'] &&
            appComponent.config['components_']['virtuals']
        ) {
            const carouselComponent = appComponent.config['components_']['virtuals'].find(
                (item) => item.id === 'Carousel3Component',
            );
            if (carouselComponent && carouselComponent['banners']) {
                images = JSON.parse(JSON.stringify(carouselComponent['banners']));
            ...
            appComponent.config[device][appComponent.country][appComponent.lngProd] &&
            appComponent.config[device][appComponent.country][appComponent.lngProd]['login'] &&
            appComponent.config[device][appComponent.country][appComponent.lngProd]['login']['virtuals']
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config[device][appComponent.country][appComponent.lngProd]['login']['virtuals'],
        ...
        if (
            ...
            appComponent.config['not_login'][appComponent.lngProd] &&
            appComponent.config['not_login'][appComponent.lngProd]['banners'] &&
            appComponent.config['not_login'][appComponent.lngProd]['banners']['banner_virtual']
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config['not_login'][appComponent.lngProd]['banners']['banner_virtual'],
            ...
            appComponent.config[device][appComponent.country][appComponent.lngProd] &&
            appComponent.config[device][appComponent.country][appComponent.lngProd]['notLogin'] &&
            appComponent.config[device][appComponent.country][appComponent.lngProd]['notLogin']['virtuals']
        ) {
            images = JSON.parse(
                JSON.stringify(
                    appComponent.config[device][appComponent.country][appComponent.lngProd]['notLogin']['virtuals'],
...
/**
 * Retrieves home icons based on the user's session state and configuration.
 * @param {Object} appComponent - The application component containing configuration.
 * @returns {Array|null} - Returns the home icons if found, otherwise null.
 */
...
/**
 * Updates the footer menu light configuration based on the appComponent.
 * @param {Object} appComponent - The application component containing configuration.
 */
export function updateMenuFooterLight(appComponent) {
    if (appComponent.config.MENU_FOOTER_LIGHT_ !== undefined) {
        appComponent.config.MENU_FOOTER_LIGHT[appComponent.config.countryL] =
            appComponent.config.MENU_FOOTER_LIGHT_;
    }
}

/**
 * Updates the providers configuration based on the appComponent.
 * @param {Object} appComponent - The application component containing configuration.
 */
export function updateProviders(appComponent) {
    if (appComponent.config.PROVIDERS_ !== undefined) {
        if (appComponent.config.PROVIDERS !== undefined && appComponent.config.PROVIDERS[appComponent.config.countryL] !== undefined) {
            appComponent.config.PROVIDERS[appComponent.config.countryL] =
                appComponent.config.PROVIDERS_;
        } else {
            appComponent.config.PROVIDERS = { [appComponent.config.countryL]: [] };
            appComponent.config.PROVIDERS[appComponent.config.countryL] =
                appComponent.config.PROVIDERS_;
...
/**
 * Retrieves sponsors from the appComponent configuration.
 * @param {Object} appComponent - The application component containing configuration.
 * @returns {Array|null} - Returns the sponsors if found, otherwise null.
 */
export function getSponsors(appComponent) {
    let SPONSORS = null;
    if (
        appComponent.config.SPONSORS_ !== undefined
    ) {
...
/**
 * Retrieves providers from the appComponent configuration.
 * @param {Object} appComponent - The application component containing configuration.
 * @returns {Array|null} - Returns the providers if found, otherwise null.
 */
 ...
 * @param {Object} appComponent - The application component containing configuration.
 * @returns {string|null} - Returns the license if found, otherwise null.
 */
export function getLicense(appComponent) {
    let LICENSE = null;
    if (
        appComponent.config.footer !== undefined &&
        appComponent.config.footer.license !== undefined
    ) {
 ...
 * @param {Object} appComponent - The application component containing configuration.
 * @returns {string|null} - Returns the download app URL if found, otherwise null.
 */
export function getUrlDownloadApp(appComponent) {
    let urlDownloadApp = null;
    if (
        appComponent.config.footer !== undefined &&
        appComponent.config.footer.downloadApp !== undefined
    ) {
...
/**
 * Retrieves the light footer menu configuration based on the appComponent.
 * @param {Object} appComponent - The application component containing configuration.
 * @returns {Array|null} - Returns the light footer menu if found, otherwise null.
 */
 ...
 * @param {Object} appComponent - The application component containing configuration.
 * @returns {Array|null} - Returns the social media links if found, otherwise null.
 */
...
/**
 * Retrieves promotional images based on the user's session state.
 * @param {Object} appComponent - The application component containing session and configuration.
 * @returns {Array|null} - Returns the promotional images if found, otherwise null.
 */
export function getPromotional2(appComponent) {
    ...
    if (
        ...
        appComponent.session.logueado != undefined &&
        ...
        appComponent.session.logueado != undefined &&
        appComponent.session.logueado &&
        appComponent.config.HOME_CARDS_IMAGES_LOGUEADO != undefined &&
        appComponent.config.HOME_CARDS_IMAGES_LOGUEADO[appComponent.country] != undefined &&
        appComponent.config.HOME_CARDS_IMAGES_LOGUEADO[appComponent.country][appComponent.lngProd] != undefined
    ) {
        PROMOTIONS = appComponent.config.HOME_CARDS_IMAGES_LOGUEADO[appComponent.country][appComponent.lngProd];
    }
    return PROMOTIONS;
}

/**
 * Retrieves jackpot information from the appComponent configuration.
 * @param {Object} appComponent - The application component containing configuration.
 * @returns {Object|null} - Returns the jackpot information if found, otherwise null.
 */
...
/**
 * Retrieves color configuration based on the device type.
 * @param {Object} appComponent - The application component containing configuration.
 * @param {string} device - The device type (e.g., mobile or desktop).
 * @returns {Object|null} - Returns the color configuration if found, otherwise null.
 */
export function getColors(appComponent, device) {
    let COLORS = null;
    if (
        appComponent.config[device] !== undefined
    ) {
