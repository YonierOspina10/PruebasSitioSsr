...
export default defineComponent({
	...
	data() {
		...
		let refreshBalance = this.$refreshBalance; // Accessing the refresh balance function
		return {
			config,
			appComponent,
			refreshBalance,
	...
	methods: {
		clickBox(box: any) {
			window.open(box.url, '_top'); // Opening the URL of the clicked box
		},
		logOut() {
			...
			this.appComponent.tokenSB = ''; // Clearing token again
			...
			try {
				(window as any).altenarWSDK.set({ token: '' }); // Setting Altenar SDK token
			} catch (e) {}
...
<template>
	...
	<div
		...
		"
		class="ticker-mobile items-baseline md:hidden text-neutral text-start md:text-end bg-base-300"
	>
