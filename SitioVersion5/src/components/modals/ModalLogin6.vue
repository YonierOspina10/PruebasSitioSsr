<template>
	<Teleport to="body">
		...
		<dialog
			class="modal w-screen h-screen bg-black/80 flex justify-center items-center"
		>
			...
			<section
				class="in relative aspect-auto h-full w-11/12 max-w-[320px] tablet:w-[800px] tablet:max-w-none tablet:max-h-[745px] max-h-[525px] tablet:max-h-none tablet:h-auto p-10 flex flex-col tablet:flex-row z-50 justify-start tablet:justify-center tablet:items-center items-start tablet:gap-4"
			>
				...
				<button
					...
					"
					class="absolute top-5 tablet:top-16 right-4 tablet:right-28 scale-110 hover:scale-125"
				>
					<svg
				...
				<div
					class="flex flex-col justify-center items-center tablet:items-start w-full max-w-[215px]"
				>
          ...
          <!-- Links for password recovery and registration -->
					...
					>
						<RouterLink
							...
							"
							class="mx-auto text-neutral font-light text-sm hover:text-neutral/75 hover:scale-105"
							>{{ $t('¿Olvidaste tu contraseña?') }}
						</RouterLink>
						<span
							class="flex justify-center items-center gap-2 cursor-default text-neutral text-sm"
							>{{ $t('¿No tienes una cuenta?')
							}}<a
								href="/registro"
								class="mx-auto text-primary font-light hover:text-primary/75 hover:scale-105 cursor-pointer"
								>{{ $t('Registrate') }}</a
							></span
						>
					</div>
				...
				<!-- Image for mobile view -->
				<img
                    :src="appComponent.config.modalLoginAssets != undefined && appComponent.config.modalLoginAssets.leftImage != undefined ? appComponent.config.modalLoginAssets.leftImage : 'https://images.virtualsoft.tech/m/msj0212T1712246738.png'"
					alt=""
					class="w-[50%] absolute bottom-5 right-0 aspect-auto z-10 tablet:hidden"
				/>
			</section>
		</dialog>
	</Teleport>
