...
import { defineComponent } from 'vue'; // Importing defineComponent for defining Vue components
...
export default defineComponent({
	...
	data() {
		...
		let login = {
			...
			password: '', // Password input for login
		};
	...
	methods: {
		...
		onSubmit(values: any) {
			...
			let data = {
				username: values.email, // Username from form
				password: values.password, // Password from form
					...
					.then((response: any) => {
						if (response.code == 0) {
							...
							try {
								...
								console.log(e); // Logging error if reporting fails
							}
							...
							let valorNumber = parseInt(timeLogin); // Parsing time to integer
							let sumMinutes = new Date(); // Creating date object for time calculation
							sumMinutes.setTime(
								valorNumber + this.config.restrictionTime * 1000 // Setting time limit
							);
							...
							this.appComponent.modal = {
								showModal: 'notification', // Setting modal to show notification
								...
								orderModal: 'closeModal', // Setting modal close order
							};
							this.appComponent.disabledLogin = false; // Enabling login button
						}
					})
					.catch((e) => {
						this.appComponent.disabledLogin = false; // Enabling login button on error
					});
			} else {
				setTimeout(() => {
		...
		signInWithGoogle(userGoogle) {
			...
			const params = {
				id_token: userGoogle.credential, // Getting ID token from Google response
				country: this.appComponent.country, // Getting country from app component
				type: 1, // Setting type for Google sign-in
			};
			this.appComponent.disabledLogin = true; // Disabling login button
			if (!this.appComponent.contingency) {
				// Checking if not in contingency mode
		...
		signInWithFacebook(userFacebook) {
			...
			const params = {
				...
				country: this.appComponent.country, // Getting country from app component
				type: 2, // Setting type for Facebook sign-in
			};
			this.appComponent.disabledLogin = true; // Disabling login button
			if (!this.appComponent.contingency) {
				// Checking if not in contingency mode
...
<template>
	...
	>
		<div class="modal-box rounded-2xl bg-white overflow-visible">
			<div
				class="flex flex-col justify-center items-center relative h-20 w-full mb-5"
			>
				<label
					for="m-modal-login"
					class="shadowlogin absolute right-1 top-1 cursor-pointer flex justify-center items-center text-3xl font-bold text-white bg-primary-content w-11 h-11 rounded-full border-3 border-solid border-white transition ease-in-out hover:scale-105"
					@click="appComponent.showModalLogin = false"
				>
					<font-awesome-icon icon="fa-solid fa-close" />
				</label>
				<h2 class="text-4xl font-bold mt-10">INICIAR SESIÓN</h2>
			</div>
			<Form
				@submit="onSubmit"
			...
			>
				<div class="form-control mb-2 mx-10">
					<label class="label pl-6">
						<span class="label-text text-base">{{
							$t('Correo Electrónico') // Label for email input
						}}</span>
					</label>
					<Field
						id="loginUsername"
					...
					>
						...
						{{ errors.email }} // Displaying email error message
					</div>
				</div>
				<div class="form-control relative mx-10">
					<label class="label pl-6">
						<span class="label-text text-base">{{
							$t('Contraseña') // Label for password input
						}}</span>
					</label>
					<Field
						class="input input-bordered shadowlogin rounded-full h-16 bg-base-200 border-2 border-solid border-white pl-6"
					...
					>
						...
						{{ errors.password }} // Displaying password error
						message
					</div>
				</div>
				<div class="grid mt-8 mx-10">
					...
					>
						{{ $t('Intente de nuevo en: ') }} // Message for retry
						countdown
						<vue-countdown
							v-if="disabledCount"
							:time="count"
						...
						>
							{{ seconds }} // Displaying remaining seconds
						</vue-countdown>
						{{ $t(' segundos.') }} // Message for seconds
					</div>
					<button
						...
						aria-label="Login"
						for="m-modal-login"
						class="btn text-black bg-primary-content w-56 mx-auto"
						v-bind:class="{
							'btn-disable': !appComponent.disabledLogin, // Adding disable class if login is disabled
						}"
					>
						{{ $t('Iniciar sesión') }} // Button text
					</button>
					<button
						v-show="appComponent.disabledLogin"
						disabled
					...
					>
						...
						{{ $t('Iniciar sesión') }} // Loading button text
					</button>
					<RouterLink
						to="/recuperar-clave"
						@click="appComponent.showModalLogin = false"
					>
						<label class="label">
							...
							<label
								for="m-modal-login"
								class="cursor-pointer text-lg hover:underline mx-auto"
				...
				<div
					class="modal-action w-full bg-primary-content rounded-b-xl h-14 flex justify-center items-center"
				>
			...
			>
				...
				<button
					...
					"
				>
					{{ $t('RESTABLECER CUENTA') }} // Button text for account
					reset
				</button>
			</div>
			<GoogleLogin
...
.modal-box {
	...
	border-radius: 10rem !important; /* Setting border radius for modal box */
}

.shadowlogin {
	box-shadow: 0px 0px 12px 0px rgba(0, 0, 0, 0.342); /* Adding shadow effect for login elements */
}
</style>
