...
export default defineComponent({
	...
	data: function () {
		let config = this.$config(); // Get configuration settings
		let appComponent: any = this.$appComponent; // Reference to the app component
		let colors: any = {}; // Object to hold color settings
		let deviceTheme: string = 'theme_colors'; // Default theme color
		return {
	...
	mounted() {}, // Lifecycle hook for additional setup after component is mounted
	methods: {
		goToLink(link) {
...
<template>
	<section class="">
		<!-- Main section for displaying banners -->
		<swiper-container
			v-if="
      ...
      `,
		...
		>
			<swiper-slide
				...
				id="swiper-slide-banners-center"
				data-analytics-label="promo:banner:mid"
				:data-analytics-id="banner.id"
        		:data-analytics-position="index + 1"
            	:data-analytics-context="`component:CarouselBannerCenterSB|layout:layout-${appComponent.config.layout}`"
			>
				<!-- Button to navigate to the banner link -->
				<button
					@click="goToLink(banner?.redirect)"
					class="carouselButton"
				>
					<img :src="banner?.url" class="bannerCenter" />
