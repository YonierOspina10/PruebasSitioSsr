...
export default defineComponent({
    ...
    let liveCasino: any = []; // Array for live casino
    ...
    if (this.appComponent.shopBonusesLoaded) {
      this.freeBets = this.appComponent.shopBonusesData.freeBets;
      this.physicalPrize = this.appComponent.shopBonusesData.physicalPrize;
      this.casinofreeSpins = this.appComponent.shopBonusesData.casinofreeSpins;
      this.liveCasino = this.appComponent.shopBonusesData.liveCasino;
      ...
      if (realCategory) {
        this.openViewBonus(realCategory);
      }
	...
	methods: {
					...
					if (respose.code == 0) {
						...
						respose.data.forEach((bono) => {
							...
							if (bono.category == 0) {
								...
								this.liveCasino.push(bono); // Add to physical prizes
							}
						});
							...
							this.config.categoryBonosLoyalty_.forEach(
								(category) => {
									if (category.id == 1) {
										...
										category.awards = this.liveCasino; // Assign casino free spins
									}
		...
		openViewBonus(category: any) {
			...
			if (
				...
				this.titleViewBonus = category.categoryName; // Set title for bonus view
      ...
      if (
        ...
        this.titleViewBonus = category.categoryName; // Set title for bonus view
        this.bonus = this.liveCasino; // Set bonuses to free bets
      }
		},
		onCloseModal() {
...
<template>
	...
	<div v-if="onModal" class="modal bg-transparent z-[999999999]">
		...
		>
			...
			<p
				v-if="message != ''"
				class="text-center text-neutral font-poppinssl"
				v-t
			>
				{{ $t(message) }}
			</p>
	...
	>
		...
		>
			...
			<div
				class="grid gap-10 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-center items-center"
			>
				<BonuseCards :data="bonus" />
...
<style scoped>
.title {
	/* Title styling with gradient background and shadow */
	...
	box-shadow: 0px 4px 4px rgba(0, 0, 0, 0.25); /* Shadow effect */
}
</style>
