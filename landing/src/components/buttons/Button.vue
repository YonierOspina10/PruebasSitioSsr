<template>
    <!-- Button component that conditionally renders based on the 'type' prop -->
    <button
        v-if="type === 'button'"
        @click="typeof action === 'function' ? action() : $router.push(action)"
        class="button-hover relative rounded-full border border-primary flex items-center justify-center hover:bg-primary"
        :style="{
            width: props.width + 'px',
            height: props.height + 'px',
        }"
    >
        <!-- SVG icon for the button -->
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width="102"
            height="58"
            viewBox="0 0 102 58"
            class="absolute -left-5 -bottom-[8px] fill-neutral-content z-1"
        >
            <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M101.224 20.782C100.252 21.8392 98.5015 21.1169 97.6033 22.3408C97.0361 24.7803 103.098 25.482 100.101 28.0005C97.7092 27.2936 98.8099 25.0895 95.8459 24.6229C93.1417 25.4647 91.1734 27.9741 89.228 30.5356C85.0983 26.7074 82.1214 33.1114 79.2809 29.6087C77.8167 30.481 76.7723 32.2763 75.0827 32.6379C74.4099 34.2174 74.7865 35.339 74.6699 35.8791C72.4074 36.2826 71.0869 37.7627 69.956 39.1364C68.1601 32.5546 62.5675 37.3623 58.7506 38.1374C56.1406 42.8331 58.6969 45.3108 58.8495 48.8172C56.9835 51.6857 53.5265 48.3802 52.8995 46.4796C48.8071 46.5174 47.3993 52.6677 46.6932 56.8104C46.2973 56.8444 45.9043 56.8709 45.6572 57.2564C42.7897 56.3452 43.2364 54.0133 39.9115 53.2929C38.6069 53.5816 38.0398 55.556 36.9715 56.3942C32.4018 53.9957 23.789 47.8804 21.0096 54.3793C20.651 54.2298 20.8994 53.819 20.9234 53.4924C21.3784 52.0686 19.9604 51.4512 19.0418 50.625C20.2188 48.0717 18.7246 46.6747 18.3186 44.8092C16.7933 44.5977 15.8032 45.6131 15.1533 47.3998C16.202 43.6244 13.1246 39.1756 9.01718 41.1548C10.6711 39.0474 9.92648 37.4021 9.55529 35.4038C8.1277 34.7222 7.79924 36.5319 6.36872 35.8578C5.88235 34.0641 8.66953 34.892 8.93205 32.6085C7.96514 31.1085 6.37367 32.9906 4.77746 32.2514C6.51262 30.9773 6.96857 28.5112 6.58145 26.5756C4.54621 25.9138 2.73634 25.7628 1.61629 27.1752C1.00629 24.8332 4.52196 25.887 5.52717 24.8775C6.63961 22.6436 3.97346 20.6224 6.81089 19.4195C7.07533 15.5599 2.5721 14.342 0.305709 13.0294C2.28962 12.3613 3.63296 14.3194 6.54592 14.6281C7.70604 14.4636 8.31887 13.0587 9.42087 12.7765C9.52017 11.5058 7.34582 11.2202 7.02656 10.1297C9.62479 9.51349 12.8806 10.3877 14.4901 7.53051C15.259 6.28167 12.8916 6.37686 13.2105 5.32174C13.5895 3.62679 15.8633 6.22615 16.6195 5.38589C17.204 4.49546 14.2181 3.59803 15.7164 3.32508C17.1405 2.33368 18.8713 4.47845 20.9856 4.12012C21.9239 3.17046 21.3012 3.02923 22.0003 1.53757C25.1782 3.87176 27.0233 2.64944 29.1085 0.926919C31.4809 4.11643 34.8758 4.61451 38.3312 3.69744C39.3914 2.45964 38.6454 1.14984 40.0915 1.40785C39.2442 3.09976 39.0875 4.49433 39.2448 5.75376C41.515 6.01671 43.3942 5.39371 44.3353 2.64438C43.4472 10.791 53.8318 14.0783 59.1698 19.5445C60.8927 18.5671 63.3468 19.247 64.2603 16.4352C67.7243 17.0896 70.7743 16.8059 73.9745 16.8484C79.818 13.3714 82.9993 5.69485 90.1853 6.19181C99.3177 6.82447 99.3462 14.6075 101.221 20.7463L101.224 20.782Z"
            />
        </svg>
        <div class="text-primary font-semibold text-lg flex items-center justify-center w-full h-full rounded-full capitalize z-10">
            <slot></slot>
        </div>
    </button>
    <!-- RouterLink component that conditionally renders based on the 'type' prop -->
    <RouterLink
        v-else-if="type === 'link'"
        :to="action"
        class="button-hover relative rounded-full border border-primary flex items-center justify-center hover:bg-primary"
        :style="{
            width: props.width + 'px',
            height: props.height + 'px',
        }"
    >
        <!-- SVG icon for the link -->
        <svg
            xmlns="http://www.w3.org/2000/svg"
            width="102"
            height="58"
            viewBox="0 0 102 58"
            class="absolute -left-5 -bottom-[8px] fill-neutral-content z-1"
        >
            <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M101.224 20.782C100.252 21.8392 98.5015 21.1169 97.6033 22.3408C97.0361 24.7803 103.098 25.482 100.101 28.0005C97.7092 27.2936 98.8099 25.0895 95.8459 24.6229C93.1417 25.4647 91.1734 27.9741 89.228 30.5356C85.0983 26.7074 82.1214 33.1114 79.2809 29.6087C77.8167 30.481 76.7723 32.2763 75.0827 32.6379C74.4099 34.2174 74.7865 35.339 74.6699 35.8791C72.4074 36.2826 71.0869 37.7627 69.956 39.1364C68.1601 32.5546 62.5675 37.3623 58.7506 38.1374C56.1406 42.8331 58.6969 45.3108 58.8495 48.8172C56.9835 51.6857 53.5265 48.3802 52.8995 46.4796C48.8071 46.5174 47.3993 52.6677 46.6932 56.8104C46.2973 56.8444 45.9043 56.8709 45.6572 57.2564C42.7897 56.3452 43.2364 54.0133 39.9115 53.2929C38.6069 53.5816 38.0398 55.556 36.9715 56.3942C32.4018 53.9957 23.789 47.8804 21.0096 54.3793C20.651 54.2298 20.8994 53.819 20.9234 53.4924C21.3784 52.0686 19.9604 51.4512 19.0418 50.625C20.2188 48.0717 18.7246 46.6747 18.3186 44.8092C16.7933 44.5977 15.8032 45.6131 15.1533 47.3998C16.202 43.6244 13.1246 39.1756 9.01718 41.1548C10.6711 39.0474 9.92648 37.4021 9.55529 35.4038C8.1277 34.7222 7.79924 36.5319 6.36872 35.8578C5.88235 34.0641 8.66953 34.892 8.93205 32.6085C7.96514 31.1085 6.37367 32.9906 4.77746 32.2514C6.51262 30.9773 6.96857 28.5112 6.58145 26.5756C4.54621 25.9138 2.73634 25.7628 1.61629 27.1752C1.00629 24.8332 4.52196 25.887 5.52717 24.8775C6.63961 22.6436 3.97346 20.6224 6.81089 19.4195C7.07533 15.5599 2.5721 14.342 0.305709 13.0294C2.28962 12.3613 3.63296 14.3194 6.54592 14.6281C7.70604 14.4636 8.31887 13.0587 9.42087 12.7765C9.52017 11.5058 7.34582 11.2202 7.02656 10.1297C9.62479 9.51349 12.8806 10.3877 14.4901 7.53051C15.259 6.28167 12.8916 6.37686 13.2105 5.32174C13.5895 3.62679 15.8633 6.22615 16.6195 5.38589C17.204 4.49546 14.2181 3.59803 15.7164 3.32508C17.1405 2.33368 18.8713 4.47845 20.9856 4.12012C21.9239 3.17046 21.3012 3.02923 22.0003 1.53757C25.1782 3.87176 27.0233 2.64944 29.1085 0.926919C31.4809 4.11643 34.8758 4.61451 38.3312 3.69744C39.3914 2.45964 38.6454 1.14984 40.0915 1.40785C39.2442 3.09976 39.0875 4.49433 39.2448 5.75376C41.515 6.01671 43.3942 5.39371 44.3353 2.64438C43.4472 10.791 53.8318 14.0783 59.1698 19.5445C60.8927 18.5671 63.3468 19.247 64.2603 16.4352C67.7243 17.0896 70.7743 16.8059 73.9745 16.8484C79.818 13.3714 82.9993 5.69485 90.1853 6.19181C99.3177 6.82447 99.3462 14.6075 101.221 20.7463L101.224 20.782Z"
            />
        </svg>
        <!-- Text container for the link -->
        <div
            class="text-primary font-semibold text-lg flex items-center justify-center w-full h-full rounded-full capitalize z-10"
        >
            <slot></slot>
            <!-- Slot for additional content -->
        </div>
    </RouterLink>
</template>
<script setup lang="ts">
/**
 * Defines the properties for a Vue component.
 * @typedef {Object} Props
 * @property {string} type - The type of the button, defaults to 'button'.
 * @property {string|Function} action - The action to be performed on button click, defaults to a console log function.
 * @property {number} width - The width of the button, defaults to 148.
 * @property {number} height - The height of the button, defaults to 45.
 */

// Define component props
const props = defineProps({
    type: {
        type: String, // Specifies the type as a String
        default: 'button', // Default value for type
    },
    action: {
        type: String || Function, // Specifies the type as String or Function
        default: () => {
            // Default function for action
            console.log('Button action not defined'); // Logs a message if action is not defined
        },
    },
    width: {
        type: Number, // Specifies the type as a Number
        default: 148, // Default value for width
    },
    height: {
        type: Number, // Specifies the type as a Number
        default: 45, // Default value for height
    },
});
</script>
<style scoped>
/* Styles for SVG inside button-hover class */
.button-hover > svg {
    display: none; /* Hide SVG by default */
    fill: hsl(var(--bc)) !important; /* Set fill color using custom property */
}

/* Styles for SVG on button-hover class when hovered */
.button-hover:hover > svg {
    display: block; /* Show SVG on hover */
    fill: hsl(var(--p)) !important; /* Change fill color on hover */
}

/* Styles for div inside button-hover class when hovered */
.button-hover:hover > div {
    --tw-text-opacity: 1 !important; /* Set text opacity */
    color: hsl(var(--bc) / var(--tw-text-opacity)) !important; /* Set text color */
}

@media (max-width: 767px) {
	.button-hover > div {
		color: hsl(var(--bc)) !important;
		background-color: hsl(var(--p)) !important;
	}
	.button-hover > svg {
		display: block !important;
		fill: hsl(var(--p)) !important;
	}

}
</style>

<!-- FILE DOCUMENTED -->
