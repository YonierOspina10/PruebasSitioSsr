<template>
    <!-- Section displayed if jackpotsList is defined and has items -->
    <section
        v-if="jackpotsList !== undefined && jackpotsList.length > 0"
        class="w-full flex justify-center items-center z-[50]"
    >
        <!-- Article container for jackpot cards -->
        <article
            class="containerJackpot w-11/12 h-full max-w-[1300px] flex flex-wrap justify-around items-center gap-2 px-2 z-[20]"
        >
            <!-- Loop through jackpotsList to render JackpotCard for each jackpot -->
            <JackpotCard
...
/**
 * Component definition for displaying jackpots.
 */
export default defineComponent({
    ...
    created() {
        // Watch for changes in appComponent's jackpotsList
        if (
    ...
    mounted() {
        // Set winner state after 2 seconds if loyalty_price is defined
        setTimeout(() => {
