...
export default defineComponent({
  components: { CloseIcon, MagnifyingGlass }, // Registering components for use in the template
  data() {
    let appComponent: any = this.$appComponent; // Reference to the app component
    let onSearch: boolean = false; // State to track if the search is active
    let searchCommand = ''; // The current search command input by the user
    return {
      loadingProgress: 0, // Progress indicator for loading
      appComponent,
      searchCommand,
      onSearch,
      results: { // Object to hold search results
        games: [],
        sportbook: {
          events: [],
        },
      ...
      urlVirtualplay: '', // URL for virtual play
    };
  },
  created() {
    ...
    this.$watch(
        ...
        (toParams) => {
          this.appComponent.gamesListSearch = []; // Clearing the games list on route change
  ...
  methods: {
    ...
    searchEnter() {
      if (this.searchCommand.length >= 3) { // Checking if the search command is at least 3 characters
        let paramns = { // Parameters for the API request
          partner_id: this.appComponent.partnerG,
          search: this.searchCommand,
          country: this.appComponent.country,
        };
        apiService.request('platform_browser', paramns).then((response: any) => {
          const mockGames = response?.casino?.games || [];
          const mockSportsEvents = response?.sportbook?.events || [];

          // Mostrar todo lo que venga del backend
          this.results.games = mockGames;
          this.results.sportbook.events = mockSportsEvents;

          // Mantener jackpots funcionando con todos los juegos
          this.handlePlaytechJackpots(mockGames);
        });

      } else {
        this.clearResults(); // Clearing results if search command is less than 3 characters
      }
    },
    handlePlaytechJackpots(filteredGames: any[]) {
      if (
          this.config.casino != undefined &&
          this.config.casino.playtechJackpot != undefined &&
          this.config.casino.playtechJackpot[this.appComponent.country] != undefined
      ) {
    ...
    debounce(func, wait) {
      ...
      return function (...args) {
        ...
        clearTimeout(timeout); // Clearing the previous timeout
        timeout = setTimeout(later, wait); // Setting a new timeout
      };
    },
    onOpenGame(game, $event) {
      if (this.appComponent.session.logueado) { // Checking if the user is logged in
        ...
        this.appComponent.openLink = true; // Indicating to open the game link
        this.appComponent.openGame(game); // Opening the game
  ...
  <div class="search-container flex flex-col gap-4">
    ...
    >
      <button
          ...
          class="bg-base-300 border-2 border-primary-content h-10 w-10 lg:h-12 lg:w-12 flex justify-center items-center rounded-full shadow-inner shadow-black/70 hover:scale-105 transition"
          aria-label="search"
          @click="
          () => {
            onSearch = !onSearch;
      ...
      <input
          v-if="onSearch"
          type="text"
          class="w-80 h-10 lg:h-12 px-6 rounded-full bg-neutral-content border-2 border-primary-content text-neutral flex hover:scale-105 hover:ml-1 transition"
    ...
    <div
        v-if="
        onSearch &&
        (results.games.length > 0 || results.sportbook.events?.length > 0)
      "
         ...
         transition-colors duration-200"
              >
                <div class="flex items-center gap-2 w-full justify-between">
                  ...
                  /> -->
                  <span
                      class="text-neutral font-poppinsm font-bold text-sm"
                  >
                    {{ event.sport }} - {{ event.country }}
                  </span>
              ...
              <div class="relative flex w-full">
                ...
                <div class="cta-button text-center z-10 relative mt-4 mb-2">
                  <a
                      ...
                      class="bg-primary text-neutral font-poppinsm font-bold py-1 px-8 rounded-full hover:bg-yellow-400 transition text-base"
                  >
                    ¡VER MÁS!
                  </a>
                </div>
              ...
              <div class="content flex flex-col items-center py-3">
                <div class="league-info flex items-center gap-2">
                  <!-- <img
                      ...
                      class="icon w-6 h-6"
                  /> -->
                  <!-- <span class="text-neutral font-poppinsm">
                    {{ event.league }}
                  </span> -->
                </div>
    ...
    <div class="flex items-center gap-1 fill-primary">
      ...
      <div v-if="results.games.length > 0" class="games grid gap-4">
        <div
        ...
        >
          <div
              class="h-32 w-full flex justify-center items-center overflow-hidden rounded-lg bg-base-300"
          >
            <img
                :src="game.icon_3"
                alt="Game Icon"
          ...
          <div
              class="content flex flex-col justify-center items-center gap-2 w-full h-full p-2 break-all"
          >
            <p
                class="game-name text-neutral text-lg text-center font-poppinsm font-bold leading-tight line-clamp-2"
            >
              {{ game.name }}
            </p>

            <button
                class="cta-button bg-base-100 text-neutral text-sm w-40 rounded-full font-bold p-2 uppercase hover:scale-105 transition"
...
@keyframes fill {
  0% {
    width: 0%; /* Starting width for the fill animation */
  }
  100% {
    width: 100%; /* Ending width for the fill animation */
  }
}
.progress-bar-container {
  ...
  background-color: rgba(209, 213, 219, 0.67); /* Background color for the progress bar */
